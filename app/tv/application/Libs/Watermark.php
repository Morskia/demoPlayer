...
class Watermark {
    ...
    private function process(){
       ...
       // $watermark = imagecreatefrompng($this->watermark);
        $watermark = imagecreatefrompng(ROOT_PATH.'/public/img/watermark.png');
        if ($watermark){
            $realImg = $this->image;

            $image = pathinfo($realImg, PATHINFO_EXTENSION)  === 'jpg' ? imagecreatefromjpeg($realImg): imagecreatefrompng($realImg) ;
            if ($image){
                switch ($this->type){
    ...
    private function watermark_center(&$image, &$watermark){
        ...
        $im_x = $size[0];
        $im_y = $size[1];
        $cof = $im_x/($watermark_x*5); // 5/1 = im_x/(wx*cof) ; wx*cof = im_x/5 ; cof = im_x/wx*5
        ...
        pathinfo($this->image, PATHINFO_EXTENSION)  === 'jpg' ? imagejpeg($image,$this->output_file,100) : imagepng($image,$this->output_file,9) ;
        imagedestroy($image);
        imagedestroy($watermark);
        return true;
    }
